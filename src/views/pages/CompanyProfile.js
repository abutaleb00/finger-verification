...
const CompanyProfile = () => {
  ...
  const companyExitOrApplication = (e) => {
    console.log(
      "state?.companyRegDate",
      moment(state?.companyRegDate).format("DD/MM/YYYY")
    );
    const sentdata = {
      companyProfile: {
        ...state,
    ...
    setBlock(true);
    axios
      .post("/doescompanyexisits", sentdata)
      .then((res) => {
    ...
    axios
      ...
      .then((res) => {
        if (res.data?.result?.error === false) {
          ...
          navigate("/nid-verify", {
            state: { userData: res.data?.data, type: 2 },
          });
          setBlock(false);
